...
def download_s2_token_fn(token, region_df):
    return download_s2_token(token, region_df)



def download_csv(wkt_polygon: str):
    # Get file name and region dataframe
    filename, region_df = get_filename_and_region_dataframe(wkt_polygon)
    ...
    download_s2_token_with_region = partial(download_s2_token_fn, region_df=region_df)

    # Download data
    with open(output_path, 'ab') as merged:
        with multiprocessing.Pool(4) as pool:
            for fname in tqdm.tqdm(pool.imap_unordered(download_s2_token_with_region, s2_tokens), total=len(s2_tokens)):
                if fname:
